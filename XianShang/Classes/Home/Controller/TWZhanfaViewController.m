...
@interface TWZhanfaViewController ()<UITextViewDelegate,IWComposeToolbarDelegate, ZLPhotoPickerBrowserViewControllerDelegate>
...
- (void)post
{
    if (self.assets.count > 0) {
        ...
        for (int i = 0; i<self.assets.count; i++) {
            NSData *data = UIImagePNGRepresentation([self.assets[i] thumbImage]);
...
- (void)setupTextView
{
    ...
    icon.image = [UIImage imageNamed:@"defaultUserIcon"];
    ...
    label.text = @"中安在线讯 据安徽商报消息，近期连降暴雨，芜湖市芜湖县一农庄内饲养的扬子鳄趁着洪灾集体“越狱”。农庄负责人介绍，农庄内共饲养92条鳄鱼，洪水涨势迅猛，超过了铁丝拦网50厘米，这才给了鳄鱼出逃的可趁之机。目前出逃扬子鳄分散在百亩水域之中，农庄工作人员已经捉回了八条。据扬子鳄养殖专家介绍，扬子鳄体型较小，脾气也没有那么火爆，不会主动攻击人。";
...
- (void)reloadScrollView{
    ...
    for (NSInteger i = 0; i < assetCount; i++) {
        ...
        btn.frame = CGRectMake(width * col, 100+row * width, width, width);
